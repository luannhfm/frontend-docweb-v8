...
export class AnalysisListComponent {
  ...
  @ViewChild('modalDictDelete') modalDictDelete?: PoModalComponent ;
    ...
    loadDicionario() {
      ...
      this.items = [];
      this.analysisService.getAll(this.page, this.pageSize).subscribe({
          next: (result: any) => {
    ...
    public onDeleteSelected() {
       ...
       for (const row of selectedRows) {
         observers.push(this.analysisService.delete(row["id_analysis"]));
       }
     ...
     generatePDF() {
      ...
      const selectedRows = this.itemsSX2.filter( (item: any) => item.$selected);
      for (const row of selectedRows) {
        this.analysisService.getDetails(row["id_analysis"]).subscribe(data => {

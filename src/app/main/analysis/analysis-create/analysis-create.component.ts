...
export class AnalysisCreateComponent implements OnInit {
  ...
  columnsFonte: PoTableColumn[] = [
    { property: "name", label: "Fontes" },
  ];
  ...
  processAnalysis() {
    ...
    this.analysisService.post(requestData).subscribe(
      (response) => {
  ...
  loadDicionario() {
    this.dicionarioService.list(1, 50).subscribe({
      next: (result: any) => {
  ...
  loadFonte() {
    ...
    this.fonteService.getFonte( this.search).subscribe({
      next: (data) => {

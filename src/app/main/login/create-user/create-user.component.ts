...
public readonly fields: Array<any> = [
  ...
  try {
    // Hash the secret key
    data.secretkey = await bcrypt.hash(data.secretkey, 10);

    // Proceed with the API call after hashing the secretkey
    ...
    console.error('Erro ao hashear a secret key', error);
    alert('Erro ao processar os dados do usuário. Por favor, tente novamente.');
  }
}
